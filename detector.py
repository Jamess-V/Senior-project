...
class MisleadingOutputDetector:
    """Fallback proxy detector used when no ground truth is provided."""

    def get_ground_truth(self, predictions, labels, correctness=None, plausibility_scores=None):
        """Create a binary proxy ground-truth label array."""
        if correctness is None:
            correctness = (predictions > 0.5) == labels

        if plausibility_scores is None:
            plausibility_scores = np.zeros_like(predictions, dtype=float)

        return np.where((~correctness) | (plausibility_scores < 0.5), 1, 0).astype(int)
